...
import bd.RemoveLikeCommentBD;

import services.tools.JSONTools;

public class RemoveLikeCommentServices {
	...
	public RemoveLikeCommentServices(Map<String, String[]> map) {
	...
	public JSONObject isSuccess() throws JSONException, SQLException, ClassNotFoundException, UnknownHostException, MongoException {
		...
		RemoveLikeCommentBD acbd = new RemoveLikeCommentBD();
		if(!acbd.checkKey(key)) return JSONTools.error("key not found or expired in the BD", -1, 400);
		
		if(!acbd.removeLike(key,idUser,idCom)) return JSONTools.error("Informations are not remove", 10000, 400);
